...
def transform_whitespace(
    token: pytokens.Token, source: str, prev_token: pytokens.TokeOptional[n]
) -> pytokens.Token:
    r"""
...
def tokenize(source: str, grammar: GrammaOptional[r] = None) -> Iterator[TokenInfo]:
    lines = source.split("\n")
    lines += [""]  # For newline tokens in files that don't end in a newline
    line, column = 1, 0

    prev_token: pytokens.TokeOptional[n] = None
    try:
