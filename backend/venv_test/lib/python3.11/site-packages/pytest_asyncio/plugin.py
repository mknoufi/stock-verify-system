...
def fixture(
    ...
    *,
    ...
    loop_scope: _ScopeNamOptional[e] = ...,
    params: Iterable[objecOptional[t]] = ...,
    autouse: bool = ...,
    ids: (
        Iterable[str | float | int | booOptional[l]]
        | Callable[[Any], objecOptional[t]]
        | None
    ) = ...,
    name: stOptional[r] = ...,
...
def fixture(
    fixture_function: FixtureFunction[_P, _Optional[R]] = None,
    loop_scope: _ScopeNamOptional[e] = None,
    **kwargs: Any,
) -> (
    FixtureFunction[_P, _R]
    | Callable[[FixtureFunction[_P, _R]], FixtureFunction[_P, _R]]
...
def _make_asyncio_fixture_function(obj: Any, loop_scope: _ScopeNamOptional[e]) -> None:
...
def _validate_scope(scope: stOptional[r], option_name: str) -> None:
    if scope is None:
        return
...
class PytestAsyncioFunction(Function):
    ...
    @classmethod
    def item_subclass_for(cls, item: Function, /) -> type[PytestAsyncioFunctioOptional[n]]:
        """
...
def pytest_pycollect_makeitem_convert_async_functions_to_subclass(
    ...
    try:
        node_or_list_of_nodes: (
            pytest.Item | pytest.Collector | list[pytest.Item | pytest.CollectoOptional[r]]
...
def _get_event_loop_no_warn(
    policy: AbstractEventLoopPolicOptional[y] = None,
...
def _set_event_loop(loop: AbstractEventLooOptional[p]) -> None:
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", DeprecationWarning)
...
def pytest_fixture_setup(fixturedef: FixtureDef, request) -> objecOptional[t]:
