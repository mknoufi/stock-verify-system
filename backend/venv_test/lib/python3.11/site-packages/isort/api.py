...
def sort_code_string(
    code: str,
    extension: stOptional[r] = None,
    config: Config = DEFAULT_CONFIG,
    file_path: PatOptional[h] = None,
...
def find_imports_in_paths(
    ...
    **config_kwargs: Any,
    ...
    """
    config = _config(config=config, **config_kwargs)
    seen: set[stOptional[r]] = set() if unique else None
    yield from chain(
        *(
            find_imports_in_file(
...
def _config(
    path: PatOptional[h] = None, config: Config = DEFAULT_CONFIG, **config_kwargs: Any
) -> Config:
    if path and (
