...
class Error:
    def __init__(
        ...
        *,
        stub_desc: stOptional[r] = None,
        runtime_desc: stOptional[r] = None,
    ) -> None:
...
@functools.lru_cache
def _module_symbol_table(runtime: types.ModuleType) -> symtable.SymbolTablOptional[e]:
...
def verify_mypyfile(
    ...
    runtime_all_as_set: set[stOptional[r]]
...
class Signature(Generic[T]):
    ...
    def __str__(self) -> str:
        ...
        def get_type(arg: Any) -> stOptional[r]:
            if isinstance(arg, inspect.Parameter):
                return None
            if isinstance(arg, nodes.Argument):
...
def _resolve_funcitem_from_decorator(dec: nodes.OverloadPart) -> nodes.FuncIteOptional[m]:
...
def safe_inspect_signature(runtime: Any) -> inspect.SignaturOptional[e]:
    if (
...
def get_mypy_node_for_name(module: str, type_name: str) -> mypy.nodes.SymbolNodOptional[e]:
...
def get_mypy_type_of_runtime_value(
    runtime: Any, type_context: mypy.types.TypOptional[e] = None
) -> mypy.types.TypOptional[e]:
    ...
    if isinstance(runtime, type) and not skip_type_object_type:
        ...
        if isinstance(type_info, nodes.TypeInfo):
            result: mypy.types.TypOptional[e] = None
...
def get_stub(module: str) -> nodes.MypyFilOptional[e]:
    """Returns a stub object for the given module, if we've built one."""
    return _all_stubs.get(module)


def get_typeshed_stdlib_modules(
    custom_typeshed_dir: stOptional[r], version_info: tuple[int, inOptional[t]] = None
) -> set[str]:
...
class _Arguments:
    ...
    mypy_config_file: stOptional[r]
    custom_typeshed_dir: stOptional[r]
    check_typeshed: bool
    version: str
    show_traceback: bool
    pdb: bool
