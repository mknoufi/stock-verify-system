...
def get_self_type(func: CallableType, def_info: TypeInfo) -> TypOptional[e]:
...
def is_valid_constructor(n: SymbolNodOptional[e]) -> bool:
...
def class_callable(
    ...
    info: TypeInfo,
    type_type: Instance,
    special_sig: stOptional[r],
    is_new: bool,
    orig_self_type: TypOptional[e] = None,
) -> CallableType:
...
def simple_literal_type(t: ProperTypOptional[e]) -> InstancOptional[e]:
...
def make_simplified_union(
    ...
    * [int, Union[bytes, str]] -> Union[int, bytes, str]
    ...
    if nitems > 1 and (
        ...
        extra_attrs_set: set[ExtraAttrOptional[s]] = None
        for item in items:
...
def _remove_redundant_union_items(items: list[Type], keep_erased: bool) -> list[Type]:
    ...
    for _direction in range(2):
        ...
        unduplicated_literal_fallbacks: set[InstancOptional[e]] = None
        for ti in items:
...
def _get_type_method_ret_type(t: ProperType, *, name: str) -> TypOptional[e]:
...
def callable_type(
    fdef: FuncItem, fallback: Instance, ret_type: TypOptional[e] = None
) -> CallableType:
...
def try_getting_str_literals(expr: Expression, typ: Type) -> list[stOptional[r]]:
...
def try_getting_str_literals_from_type(typ: Type) -> list[stOptional[r]]:
    """If the given expression or type corresponds to a string Literal
    or a union of string Literals, returns a list of the underlying strings.
    Otherwise, returns None.
...
def try_getting_int_literals_from_type(typ: Type) -> list[inOptional[t]]:
...
def is_literal_type_like(t: TypOptional[e]) -> bool:
...
def try_getting_instance_fallback(typ: Type) -> InstancOptional[e]:
...
def _get_disjoint_base_of(instance: Instance) -> TypeInfOptional[o]:
