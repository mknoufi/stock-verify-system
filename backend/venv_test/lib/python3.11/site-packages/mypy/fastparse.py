...
def parse_type_ignore_tag(tag: stOptional[r]) -> list[stOptional[r]]:
...
def parse_type_comment(
    type_comment: str, line: int, column: int, errors: ErrorOptional[s]
) -> tuple[list[stOptional[r]], ProperTypOptional[e]]:
    ...
    try:
        ...
        if extra_ignore:
            tag: stOptional[r] = extra_ignore.group(1)
            ignored: list[stOptional[r]] = parse_type_ignore_tag(tag)
            if ignored is None:
                if errors is not None:
                    errors.report(
...
def find_disallowed_expression_in_annotation_scope(expr: ast3.expOptional[r]) -> ast3.expOptional[r]:
    if expr is None:
        return None
...
class ASTConverter:
    def __init__(
        ...
        *,
        ...
        self.visitor_cache: dict[type, Callable[[ASOptional[T]], Any]] = {}
    ...
    def visit(self, node: ASOptional[T]) -> Any:
        if node is None:
            return None
        typeobj = type(node)
    ...
    def translate_opt_expr_list(self, l: Sequence[ASOptional[T]]) -> list[ExpressioOptional[n]]:
        res: list[ExpressioOptional[n]] = []
        for e in l:
            exp = self.visit(e)
            res.append(exp)
        return res
    ...
    def translate_type_comment(
        self, n: ast3.stmt | ast3.arg, type_comment: stOptional[r]
    ) -> ProperTypOptional[e]:
    ...
    def as_block(self, stmts: list[ast3.stmt]) -> BlocOptional[k]:
        b = None
        if stmts:
    ...
    def fix_function_overloads(self, stmts: list[Statement]) -> list[Statement]:
        ...
        current_overload_name: stOptional[r] = None
        last_unconditional_func_def: stOptional[r] = None
        last_if_stmt: IfStmOptional[t] = None
        last_if_overload: Decorator | FuncDef | OverloadedFuncDeOptional[f] = None
        last_if_stmt_overload_name: stOptional[r] = None
        last_if_unknown_truth_value: IfStmOptional[t] = None
        skipped_if_stmts: list[IfStmt] = []
        for stmt in stmts:
            if_overload_name: stOptional[r] = None
            if_block_with_overload: BlocOptional[k] = None
            if_unknown_truth_value: IfStmOptional[t] = None
    ...
    def _check_ifstmt_for_overloads(
        self, stmt: IfStmt, current_overload_name: stOptional[r] = None
    ) -> stOptional[r]:
    ...
    def _get_executable_if_block_with_overloads(
        self, stmt: IfStmt
    ) -> tuple[BlocOptional[k], IfStmOptional[t]]:
    ...
    def do_func_def(
        ...
        explicit_type_params: list[TypeParaOptional[m]] = None

        arg_types: list[TypOptional[e]] = []
        if no_type_check:
    ...
    def set_type_optional(self, type: TypOptional[e], initializer: ExpressioOptional[n]) -> None:
    ...
    def visit_Import(self, n: ast3.Import) -> Import:
        names: list[tuple[str, stOptional[r]]] = []
...
class TypeConverter:
    ...
    def invalid_type(self, node: AST, note: stOptional[r] = None) -> RawExpressionType:
    ...
    @overload
    def visit(self, node: ASOptional[T]) -> ProperTypOptional[e]: ...
    ...
    def _extract_argument_name(self, n: ast3.expr) -> stOptional[r]:
    ...
    def numeric_type(self, value: object, n: AST) -> Type:
        ...
        if isinstance(value, int):
            numeric_value: inOptional[t] = value
...
def stringify_name(n: AST) -> stOptional[r]:
