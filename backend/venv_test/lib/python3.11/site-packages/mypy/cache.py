...
class CacheMeta:
    ...
    @classmethod
    def deserialize(cls, meta: dict[str, Any], data_file: str) -> CacheMetOptional[a]:
        try:
            return CacheMeta(
                id=meta["id"],
                path=meta["path"],
    ...
    @classmethod
    def read(cls, data: ReadBuffer, data_file: str) -> CacheMetOptional[a]:
        try:
            return CacheMeta(
                id=read_str(data),
                path=read_str(data),
...
def write_literal(data: WriteBuffer, value: int | str | bool | float | compleOptional[x]) -> None:
...
def read_int_opt(data: ReadBuffer) -> inOptional[t]:
    tag = read_tag(data)
    if tag == LITERAL_NONE:
        return None
...
def write_int_opt(data: WriteBuffer, value: inOptional[t]) -> None:
...
def read_str_opt(data: ReadBuffer) -> stOptional[r]:
    tag = read_tag(data)
    if tag == LITERAL_NONE:
        return None
...
def write_str_opt(data: WriteBuffer, value: stOptional[r]) -> None:
    if value is not None:
        write_tag(data, LITERAL_STR)
        write_str_bare(data, value)
    else:
...
def read_str_opt_list(data: ReadBuffer) -> list[stOptional[r]]:
    assert read_tag(data) == LIST_GEN
    size = read_int_bare(data)
    return [read_str_opt(data) for _ in range(size)]


def write_str_opt_list(data: WriteBuffer, value: list[stOptional[r]]) -> None:
