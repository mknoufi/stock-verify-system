...
class FineGrainedBuildManager:
    def __init__(self, result: BuildResult) -> None:
        ...
        self.blocking_error: tuple[str, stOptional[r]] = None
    ...
    def update_one(
        ...
        initial_set: set[str],
        removed_set: set[str],
        blocking_error: stOptional[r],
        followed: bool,
    ) -> tuple[list[tuple[str, str]], tuple[str, str], list[stOptional[r]]]:
...
def update_module_isolated(
    ...
    new_modules_dict: dict[str, MypyFilOptional[e]] = {module: state.tree}
...
def calculate_active_triggers(
    manager: BuildManager,
    old_snapshots: dict[str, dict[str, SymbolSnapshot]],
    new_modules: dict[str, MypyFilOptional[e]],
) -> set[str]:
...
def replace_modules_with_new_variants(
    ...
    old_modules: dict[str, MypyFilOptional[e]],
    new_modules: dict[str, MypyFilOptional[e]],
) -> None:
...
def lookup_target(
...
) -> tuple[list[FineGrainedDeferredNode], TypeInfOptional[o]]:
    ...
    if rest:
        ...
        components = []
    node: SymbolNodOptional[e] = modules[module]
    file: MypyFilOptional[e] = None
    active_class = None
    for c in components:
    ...
    if isinstance(node, TypeInfo):
        ...
        stale_info: TypeInfOptional[o] = None
        if node.is_protocol:
            stale_info = node
...
def target_from_node(module: str, node: FuncDef | MypyFile | OverloadedFuncDef) -> stOptional[r]:
...
def extract_fnam_from_message(message: str) -> stOptional[r]:
