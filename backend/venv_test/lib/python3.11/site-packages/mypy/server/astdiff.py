...
def snapshot_definition(node: SymbolNodOptional[e], common: SymbolSnapshot) -> SymbolSnapshot:
    ...
    if isinstance(node, SYMBOL_FUNCBASE_TYPES):
        ...
        deprecated: str | list[stOptional[r]] | None = None
        if isinstance(node, FuncDef):
...
def snapshot_optional_type(typ: TypOptional[e]) -> SnapshotItem:
    if typ:
...
def encode_optional_str(s: stOptional[r]) -> str:
    if s is None:
        return "<None>"
    else:
        return s
