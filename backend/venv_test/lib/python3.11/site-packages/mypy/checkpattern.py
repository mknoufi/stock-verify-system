...
class PatternChecker(PatternVisitor[PatternType]):
    ...
    def visit_sequence_pattern(self, o: SequencePattern) -> PatternType:
        ...
        star_position: inOptional[t] = None
    ...
    def get_sequence_type(self, t: Type, context: Context) -> TypOptional[e]:
        t = get_proper_type(t)
    ...
    def contract_starred_pattern_types(
        self, types: list[Type], star_pos: inOptional[t], num_patterns: int
    ) -> list[Type]:
        """
    ...
    def expand_starred_pattern_types(
        self, types: list[Type], star_pos: inOptional[t], num_types: int, original_unpack: bool
    ) -> list[Type]:
    ...
    def get_mapping_item_type(
        ...
        if isinstance(mapping_type, TypedDictType):
            with self.msg.filter_errors() as local_errors:
                result: TypOptional[e] = self.chk.expr_checker.visit_typeddict_index_expr(
                    mapping_type, key
                )[0]
    ...
    def visit_class_pattern(self, o: ClassPattern) -> PatternType:
        ...
        #
        keyword_pairs: list[tuple[stOptional[r], Pattern]] = []
...
def get_match_arg_names(typ: TupleType) -> list[stOptional[r]]:
    args: list[stOptional[r]] = []
