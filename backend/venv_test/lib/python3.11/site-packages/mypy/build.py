...
def build(
    ...
    options: Options,
    alt_lib_path: stOptional[r] = None,
    flush_errors: Callable[[stOptional[r], list[str], bool], NonOptional[e]] = None,
    fscache: FileSystemCachOptional[e] = None,
    stdout: TextIOptional[O] = None,
    stderr: TextIOptional[O] = None,
    extra_plugins: Sequence[PlugiOptional[n]] = None,
) -> BuildResult:
    """Analyze a program.
    ...
    def default_flush_errors(
        filename: stOptional[r], new_messages: list[str], is_serious: bool
    ) -> None:
...
def _build(
    sources: list[BuildSource],
    options: Options,
    alt_lib_path: stOptional[r],
    flush_errors: Callable[[stOptional[r], list[str], bool], None],
    fscache: FileSystemCachOptional[e],
    stdout: TextIO,
    stderr: TextIO,
...
class BuildManager:
    ...
    def __init__(
        ...
        stdout: TextIO,
        stderr: TextIO,
        error_formatter: ErrorFormatteOptional[r] = None,
    ) -> None:
        ...
        self.shadow_equivalence_map: dict[str, stOptional[r]] = {}
        self.plugin = plugin
    ...
    def get_stat(self, path: str) -> os.stat_resulOptional[t]:
...
def read_plugins_snapshot(manager: BuildManager) -> dict[str, stOptional[r]]:
...
) -> dict[str, tuple[float, int, stOptional[r]]]:
    quickstart: dict[str, tuple[float, int, stOptional[r]]] = None
...
def read_deps_cache(manager: BuildManager, graph: Graph) -> dict[str, FgDepMetOptional[a]]:
...
def _load_ff_file(file: str, manager: BuildManager, log_error: str) -> byteOptional[s]:
    t0 = time.time()
    try:
...
def get_cache_names(id: str, path: str, options: Options) -> tuple[str, str, stOptional[r]]:
...
def find_cache_meta(id: str, path: str, manager: BuildManager) -> CacheMetOptional[a]:
    ...
    meta: bytes | dict[str, AnOptional[y]]
    t0 = time.time()
...
def validate_meta(
    meta: CacheMetOptional[a], id: str, path: stOptional[r], ignore_all: bool, manager: BuildManager
) -> CacheMetOptional[a]:
...
def write_cache(
    ...
    ignore_all: bool,
    manager: BuildManager,
) -> tuple[bytes, tuple[CacheMeta, stOptional[r]]]:
...
class State:
    ...
    path: stOptional[r] = None  # Path to module source
    abspath: stOptional[r] = None  # Absolute path to module source
    xpath: str  # Path or '<string>'
    source: stOptional[r] = None  # Module source code
    source_hash: stOptional[r] = None  # Hash calculated based on the source code
    meta_source_hash: stOptional[r] = None  # Hash of the source given in the meta, if any
    meta: CacheMetOptional[a] = None
    data: stOptional[r] = None
    tree: MypyFilOptional[e] = None
    ...
    ancestors: list[stOptional[r]] = None
    ...
    _type_checker: TypeCheckeOptional[r] = None
    ...
    def __init__(
        ...
        source: stOptional[r],
        manager: BuildManager,
        caller_state: StatOptional[e] = None,
        caller_line: int = 0,
        ancestor_for: StatOptional[e] = None,
    ...
    def load_tree(self, temporary: bool = False) -> None:
        ...
        data: bytes | dict[str, AnOptional[y]]
    ...
    def write_cache(self) -> tuple[CacheMeta, stOptional[r]]:
        assert self.tree is not None, "Internal error: method must be called on parsed file only"
        ...
        for dep in all_deps:
            ...
            try:
                if dep in self.ancestors:
                    state: StatOptional[e] = None
                    ancestor: StatOptional[e] = self
                else:
...
def find_module_simple(id: str, manager: BuildManager) -> stOptional[r]:
...
def in_partial_package(id: str, manager: BuildManager) -> bool:
    ...
    while "." in id:
        ...
        if parent in manager.modules:
            parent_mod: MypyFilOptional[e] = manager.modules[parent]
        else:
...
def skipping_module(
    manager: BuildManager, line: int, caller_state: StatOptional[e], id: str, path: str
) -> None:
...
def dump_graph(graph: Graph, stdout: TextIOptional[O] = None) -> None:
...
def load_graph(
    sources: list[BuildSource],
    manager: BuildManager,
    old_graph: GrapOptional[h] = None,
    new_modules: list[StatOptional[e]] = None,
) -> Graph:
