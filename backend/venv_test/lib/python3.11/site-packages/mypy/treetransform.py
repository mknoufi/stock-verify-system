...
class TransformVisitor(NodeVisitor[Node]):
    ...
    def optional_expr(self, expr: ExpressioOptional[n]) -> ExpressioOptional[n]:
        if expr:
    ...
    def optional_block(self, block: BlocOptional[k]) -> BlocOptional[k]:
        if block:
    ...
    def optional_expressions(
        self, expressions: Iterable[ExpressioOptional[n]]
    ) -> list[ExpressioOptional[n]]:
    ...
    def optional_names(self, names: Iterable[NameExpOptional[r]]) -> list[NameExpOptional[r]]:
        result: list[NameExpOptional[r]] = []
        for name in names:
            if name:
    ...
    def optional_type(self, type: TypOptional[e]) -> TypOptional[e]:
        if type:
