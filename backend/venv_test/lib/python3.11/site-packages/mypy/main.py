...
def run_build(
    ...
    stdout: TextIO,
    stderr: TextIO,
) -> tuple[build.BuildResulOptional[t], list[str], bool]:
    formatter = util.FancyFormatter(
        stdout, stderr, options.hide_error_codes, hide_success=bool(options.output)
    )
    ...
    def flush_errors(filename: stOptional[r], new_messages: list[str], serious: bool) -> None:
        if options.pretty:
...
class CapturableArgumentParser(argparse.ArgumentParser):
    ...
    # =====================
    def print_usage(self, file: SupportsWrite[stOptional[r]] = None) -> None:
        if file is None:
            file = self.stdout
        self._print_message(self.format_usage(), file)

    def print_help(self, file: SupportsWrite[stOptional[r]] = None) -> None:
        if file is None:
            file = self.stdout
        self._print_message(self.format_help(), file)

    def _print_message(self, message: str, file: SupportsWrite[stOptional[r]] = None) -> None:
        if message:
            if file is None:
                file = self.stderr
            file.write(message)
    ...
    # Exiting methods
    # ===============
    def exit(self, status: int = 0, message: stOptional[r] = None) -> NoReturn:
        if message:
...
class CapturableVersionAction(argparse.Action):
    ...
    def __call__(
        ...
        values: str | Sequence[AnOptional[y]],
...
def define_options(
    ...
    def add_invertible_flag(
        flag: str,
        *,
        inverse: stOptional[r] = None,
        default: bool,
        dest: stOptional[r] = None,
        help: str,
        strict_flag: bool = False,
        group: argparse._ActionsContaineOptional[r] = None,
    ) -> None:
        if inverse is None:
...
def process_package_roots(
    fscache: FileSystemCachOptional[e], parser: argparse.ArgumentParser, options: Options
) -> None:
...
def maybe_write_junit_xml(
    ...
    messages_by_file: dict[stOptional[r], list[str]],
    options: Options,
) -> None:
