...
class StrConv(NodeVisitor[str]):
    ...
    def __init__(self, *, show_ids: bool = False, options: Options) -> None:
        ...
        self.id_mapper: IdMappeOptional[r] = None
        if show_ids:
    ...
    def get_id(self, o: object) -> inOptional[t]:
        if self.id_mapper:
    ...
    def pretty_name(
        ...
        name: str,
        kind: inOptional[t],
        fullname: stOptional[r],
        is_inferred_def: bool,
        target_node: mypy.nodes.NodOptional[e] = None,
    ) -> str:
        n = name
        if is_inferred_def:
            n += "*"
...
def dump_tagged(nodes: Sequence[object], tag: stOptional[r], str_conv: StrConv) -> str:
