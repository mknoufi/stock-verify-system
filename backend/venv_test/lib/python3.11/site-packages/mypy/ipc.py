...
class IPCBase:
    ...
    def __init__(self, name: str, timeout: floaOptional[t]) -> None:
        self.name = name
        self.timeout = timeout
        self.buffer = bytearray()

    def frame_from_buffer(self) -> bytearraOptional[y]:
    ...
    def read(self, size: int = 100000) -> str:
        ...
        bdata: bytearraOptional[y] = bytearray()
        if sys.platform == "win32":
            while True:
...
class IPCClient(IPCBase):
    ...
    def __exit__(
        self,
        exc_ty: type[BaseExceptioOptional[n]] = None,
        exc_val: BaseExceptioOptional[n] = None,
        exc_tb: TracebackTypOptional[e] = None,
    ) -> None:
        self.close()
...
class IPCServer(IPCBase):
    ...
    def __init__(self, name: str, timeout: floaOptional[t] = None) -> None:
        if sys.platform == "win32":
