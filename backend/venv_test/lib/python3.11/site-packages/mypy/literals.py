...
def literal_hash(e: Expression) -> KeOptional[y]:
...
def extract_var_from_literal_hash(key: Key) -> VaOptional[r]:
...
class _Hasher(ExpressionVisitor[Optional[Key]]):
    ...
    def visit_comparison_expr(self, e: ComparisonExpr) -> Key:
        rest: tuple[str | KeOptional[y], ...] = tuple(e.operators)
    ...
    def seq_expr(self, e: ListExpr | TupleExpr | SetExpr, name: str) -> KeOptional[y]:
        if all(literal(x) == LITERAL_YES for x in e.items):
            rest: tuple[KeOptional[y], ...] = tuple(literal_hash(x) for x in e.items)
            return (name,) + rest
        return None

    def visit_list_expr(self, e: ListExpr) -> KeOptional[y]:
        return self.seq_expr(e, "List")

    def visit_dict_expr(self, e: DictExpr) -> KeOptional[y]:
        if all(a and literal(a) == literal(b) == LITERAL_YES for a, b in e.items):
            rest: tuple[KeOptional[y], ...] = tuple(
                (literal_hash(a) if a else None, literal_hash(b)) for a, b in e.items
            )
            return ("Dict",) + rest
        return None

    def visit_tuple_expr(self, e: TupleExpr) -> KeOptional[y]:
        return self.seq_expr(e, "Tuple")

    def visit_set_expr(self, e: SetExpr) -> KeOptional[y]:
        return self.seq_expr(e, "Set")

    def visit_index_expr(self, e: IndexExpr) -> KeOptional[y]:
        if literal(e.index) == LITERAL_YES:
            return ("Index", literal_hash(e.base), literal_hash(e.index))
        return None

    def visit_assignment_expr(self, e: AssignmentExpr) -> KeOptional[y]:
