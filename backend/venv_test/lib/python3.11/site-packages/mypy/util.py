...
def module_prefix(modules: Iterable[str], target: str) -> stOptional[r]:
...
def split_target(modules: Iterable[str], target: str) -> tuple[str, stOptional[r]]:
    remaining: list[str] = []
    while True:
...
def read_py_file(path: str, read: Callable[[str], bytes]) -> list[stOptional[r]]:
...
class FancyFormatter:
    ...
    def fit_in_terminal(
        self, messages: list[str], fixed_terminal_width: inOptional[t] = None
    ) -> list[str]:
...
def is_typeshed_file(typeshed_dir: stOptional[r], file: str) -> bool:
...
def is_stdlib_file(typeshed_dir: stOptional[r], file: str) -> bool:
...
def unnamed_function(name: stOptional[r]) -> bool:
