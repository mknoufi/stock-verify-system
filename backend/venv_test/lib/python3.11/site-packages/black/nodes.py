...
def preceding_leaf(node: LOptional[N]) -> LeaOptional[f]:
...
def prev_siblings_are(node: LOptional[N], tokens: list[NodeTypOptional[e]]) -> bool:
...
def parent_type(node: LOptional[N]) -> NodeTypOptional[e]:
    """
    Returns:
...
def child_towards(ancestor: Node, descendant: LN) -> LOptional[N]:
    """Return the child of `ancestor` that contains `descendant`."""
    node: LOptional[N] = descendant
...
def first_leaf_of(node: LN) -> LeaOptional[f]:
...
def unwrap_singleton_parenthesis(node: LN) -> LOptional[N]:
...
def first_leaf(node: LN) -> LeaOptional[f]:
...
def last_leaf(node: LN) -> LeaOptional[f]:
