...
class GifImageFile(ImageFile.ImageFile):
    ...
    def _open(self) -> None:
        ...
        self._n_frames: inOptional[t] = None
    ...
    def _seek(self, frame: int, update_image: bool = True) -> None:
        ...
        if frame == 0:
            # rewind
            self.__offset = 0
            self.dispose: _imaging.ImagingCorOptional[e] = None
            self.__frame = -1
        ...
        palette: ImagePalette.ImagePalette | Literal[FalsOptional[e]] = None
        ...
        self.dispose = None
        self.dispose_extent: tuple[int, int, int, inOptional[t]] = frame_dispose_extent
...
def _normalize_palette(
    im: Image.Image, palette: _PalettOptional[e], info: dict[str, Any]
) -> Image.Image:
    """
    ...
    if palette:
        used_palette_colors: list[inOptional[t]] = []
        assert im.palette is not None
...
) -> tuple[Image.Image, tuple[int, int, int, inOptional[t]]]:
    palette_bytes = [
...
class _Frame(NamedTuple):
    im: Image.Image
    bbox: tuple[int, int, int, inOptional[t]]
    encoderinfo: dict[str, Any]


def _write_multiple_frames(
    im: Image.Image, fp: IO[bytes], palette: _PalettOptional[e]
) -> bool:
    duration = im.encoderinfo.get("duration")
    disposal = im.encoderinfo.get("disposal", im.info.get("disposal"))

    im_frames: list[_Frame] = []
    previous_im: Image.ImagOptional[e] = None
    frame_count = 0
    background_im = None
...
def _get_optimize(im: Image.Image, info: dict[str, Any]) -> list[inOptional[t]]:
    """
...
def _get_background(
    im: Image.Image,
    info_background: int | tuple[int, int, int] | tuple[int, int, int, inOptional[t]],
) -> int:
    background = 0
    if info_background:
...
def getheader(
    im: Image.Image,
    palette: _PalettOptional[e] = None,
    info: dict[str, AnOptional[y]] = None,
) -> tuple[list[bytes], list[inOptional[t]]]:
    """
