...
class JpegImageFile(ImageFile.ImageFile):
    ...
    def _getmp(self) -> dict[int, AnOptional[y]]:
        return _getmp(self)


def _getexif(self: JpegImageFile) -> dict[int, AnOptional[y]]:
    if "exif" not in self.info:
        return None
    return self.getexif()._get_merged_dict()


def _getmp(self: JpegImageFile) -> dict[int, AnOptional[y]]:
...
def _save(im: Image.Image, fp: IO[bytes], filename: str | bytes) -> None:
    ...
    def validate_qtables(
        qtables: (
            ...
            | tuple[list[int], ...]
            | list[list[int]]
            | dict[int, list[inOptional[t]]]
        ),
    ) -> list[list[inOptional[t]]]:
        if qtables is None:
            return qtables
...
def jpeg_factory(
    fp: IO[bytes], filename: str | byteOptional[s] = None
