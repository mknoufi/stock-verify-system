...
class PdfParser:
    ...
    def __init__(
        self,
        filename: stOptional[r] = None,
        f: IO[byteOptional[s]] = None,
        buf: bytes | bytearraOptional[y] = None,
        start_offset: int = 0,
        mode: str = "rb",
    ) -> None:
        if buf and f:
        ...
        self.buf: bytes | bytearray | mmap.mmaOptional[p] = buf
        self.f = f
        ...
        self.root_ref: IndirectReferencOptional[e]
        self.info_ref: IndirectReferencOptional[e]
        self.pages_ref: IndirectReferencOptional[e]
        self.last_xref_section_offset: inOptional[t]
        if self.buf:
            self.read_pdf_info()
        else:
    ...
    def write_xref_and_trailer(
        self, new_root_ref: IndirectReferencOptional[e] = None
    ) -> None:
        assert self.f is not None
        if new_root_ref:
            self.del_root()
    ...
    def write_page(
        self, ref: int | IndirectReferencOptional[e], *objs: Any, **dict_obj: Any
    ) -> IndirectReference:
    ...
    def write_obj(
        self, ref: IndirectReferencOptional[e], *objs: Any, **dict_obj: Any
    ) -> IndirectReference:
        assert self.f is not None
        f = self.f
        if ref is None:
    ...
    def next_object_id(self, offset: inOptional[t] = None) -> IndirectReference:
        try:
    ...
    def get_value(
        ...
        offset: int,
        expect_indirect: IndirectReferencOptional[e] = None,
        max_nesting: int = -1,
    ) -> tuple[Any, inOptional[t]]:
        if max_nesting == 0:
            return None, None
        ...
        if m:
            ...
            current_offset: inOptional[t] = offset
            while not m:
    ...
    def linearize_page_tree(
        self, node: PdfDicOptional[t] = None
