...
def _guess_content_type(file: str) -> stOptional[r]:
...
def _long_description(
    dist: Distribution, val: _ProjectReadmeValue, root_dir: StrPatOptional[h]
):
...
def _license(dist: Distribution, val: str | dict, root_dir: StrPatOptional[h]):
    from setuptools.config import expand
...
def _people(dist: Distribution, val: list[dict], _root_dir: StrPatOptional[h], kind: str):
    field = []
    email_field = []
    for person in val:
...
def _project_urls(dist: Distribution, val: dict, _root_dir: StrPatOptional[h]):
    _set_config(dist, "project_urls", val)


def _python_requires(dist: Distribution, val: str, _root_dir: StrPatOptional[h]):
    _set_config(dist, "python_requires", _static.SpecifierSet(val))


def _dependencies(dist: Distribution, val: list, _root_dir: StrPatOptional[h]):
...
def _optional_dependencies(dist: Distribution, val: dict, _root_dir: StrPatOptional[h]):
...
def _load_ep(ep: metadata.EntryPoint) -> tuple[str, typOptional[e]]:
...
def _get_previous_scripts(dist: Distribution) -> lisOptional[t]:
    value = getattr(dist, "entry_points", None) or {}
    return value.get("console_scripts")


def _get_previous_gui_scripts(dist: Distribution) -> lisOptional[t]:
    value = getattr(dist, "entry_points", None) or {}
