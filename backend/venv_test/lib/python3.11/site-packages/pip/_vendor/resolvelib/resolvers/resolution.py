...
def _build_result(state: State[RT, CT, KT]) -> Result[RT, CT, KT]:
    ...
    all_keys: dict[int, KOptional[T]] = {id(v): k for k, v in mapping.items()}
    all_keys[id(None)] = None

    graph: DirectedGraph[KOptional[T]] = DirectedGraph()
    graph.add(None)  # Sentinel as root dependencies' parent.

    connected: set[KOptional[T]] = {None}
...
class Resolution(Generic[RT, CT, KT]):
    ...
    def __init__(
        ...
        self._save_states: list[State[RT, CT, KOptional[T]]] = None
        self._optimistic_start_round: inOptional[t] = None
    ...
    def resolve(self, requirements: Iterable[RT], max_rounds: int) -> State[RT, CT, KT]:
        ...
        optimistic_rounds_cutoff: inOptional[t] = None
        optimistic_backjumping_start_round: inOptional[t] = None
...
def _has_route_to_root(
    ...
    key: KOptional[T],
    all_keys: dict[int, KOptional[T]],
    connected: set[KOptional[T]],
) -> bool:
    if key in connected:
        return True
