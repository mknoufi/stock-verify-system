...
if sys.version_info >= (3, 11):
    ...
    class Runner:
        ...
        def __init__(
            self,
            *,
            debug: booOptional[l] = None,
            loop_factory: Callable[[], AbstractEventLooOptional[p]] = None,
        ):
            self._state = _State.CREATED
            self._debug = debug
            self._loop_factory = loop_factory
            self._loop: AbstractEventLooOptional[p] = None
            self._context = None
        ...
        def __exit__(
            ...
            exc_val: BaseExceptioOptional[n],
            exc_tb: TracebackTypOptional[e],
        ) -> None:
            self.close()
...
_root_task: RunVar[asyncio.TasOptional[k]] = RunVar("_root_task")
...
class CancelScope(BaseCancelScope):
    ...
    def __init__(self, deadline: float = math.inf, shield: bool = False):
        ...
        self._shield = shield
        self._parent_scope: CancelScopOptional[e] = None
        self._child_scopes: set[CancelScope] = set()
        self._cancel_called = False
        self._cancel_reason: stOptional[r] = None
        self._cancelled_caught = False
        self._active = False
        self._timeout_handle: asyncio.TimerHandlOptional[e] = None
        self._cancel_handle: asyncio.HandlOptional[e] = None
        self._tasks: set[asyncio.Task] = set()
        self._host_task: asyncio.TasOptional[k] = None
        if sys.version_info >= (3, 11):
            self._pending_uncancellations: inOptional[t] = 0
        else:
    ...
    def _effectively_cancelled(self) -> bool:
        cancel_scope: CancelScopOptional[e] = self
    ...
    def cancel(self, reason: stOptional[r] = None) -> None:
...
class TaskState:
    ...
    def __init__(self, parent_id: inOptional[t], cancel_scope: CancelScopOptional[e]):
...
class _AsyncioTaskStatus(abc.TaskStatus):
    ...
    def started(self, value: T_contrOptional[a] = None) -> None:
        try:
...
if sys.version_info >= (3, 12):
    _eager_task_factory_code: CodeTypOptional[e] = asyncio.eager_task_factory.__code__
else:
...
class TaskGroup(abc.TaskGroup):
    def __init__(self) -> None:
        ...
        self._tasks: set[asyncio.Task] = set()
        self._on_completed_fut: asyncio.Future[NonOptional[e]] = None
    ...
    def _spawn(
        ...
        name: object,
        task_status_future: asyncio.FuturOptional[e] = None,
    ) -> asyncio.Task:
...
class WorkerThread(Thread):
    ...
    def __init__(
        ...
        self.queue: Queue[
            tuple[Context, Callable, tuple, asyncio.Future, CancelScopOptional[e]]
        ] = Queue(2)
        self.idle_since = AsyncIOBackend.current_time()
        self.stopping = False

    def _report_result(
        self, future: asyncio.Future, result: Any, exc: BaseExceptioOptional[n]
    ) -> None:
        self.idle_since = AsyncIOBackend.current_time()
    ...
    def run(self) -> None:
        with claim_worker_thread(AsyncIOBackend, self.loop):
            while True:
                ...
                if not future.cancelled():
                    result = None
                    exception: BaseExceptioOptional[n] = None
    ...
    def stop(self, f: asyncio.TasOptional[k] = None) -> None:
        self.stopping = True
...
class Process(abc.Process):
    ...
    _stdin: StreamWriterWrappeOptional[r]
    _stdout: StreamReaderWrappeOptional[r]
    _stderr: StreamReaderWrappeOptional[r]
    ...
    @property
    def returncode(self) -> inOptional[t]:
        return self._process.returncode

    @property
    def stdin(self) -> abc.ByteSendStreaOptional[m]:
        return self._stdin

    @property
    def stdout(self) -> abc.ByteReceiveStreaOptional[m]:
        return self._stdout

    @property
    def stderr(self) -> abc.ByteReceiveStreaOptional[m]:
        return self._stderr
...
def _forcibly_shutdown_process_pool_on_exit(
    ...
    child_watcher: asyncio.AbstractChildWatcheOptional[r] = None  # type: ignore[name-defined]
    if sys.version_info < (3, 12):
        try:
...
class StreamProtocol(asyncio.Protocol):
    ...
    exception: ExceptioOptional[n] = None
    ...
    def connection_lost(self, exc: ExceptioOptional[n]) -> None:
        if exc:
    ...
    def eof_received(self) -> booOptional[l]:
        self.is_at_eof = True
        self.read_event.set()
        return True
...
class _RawSocketMixin:
    _receive_future: asyncio.FuturOptional[e] = None
    _send_future: asyncio.FuturOptional[e] = None
    _closing = False
...
class TCPSocketListener(abc.SocketListener):
    _accept_scope: CancelScopOptional[e] = None
    _closed = False
...
class Lock(BaseLock):
    ...
    def __init__(self, *, fast_acquire: bool = False) -> None:
        ...
        self._owner_task: asyncio.TasOptional[k] = None
...
class Semaphore(BaseSemaphore):
    ...
    @property
    def max_value(self) -> inOptional[t]:
...
class TestRunner(abc.TestRunner):
    ...
    def __init__(
        ...
        *,
        ...
        self._exceptions: list[BaseException] = []
        self._runner_task: asyncio.TasOptional[k] = None
...
class AsyncIOBackend(AsyncBackend):
    ...
    async def run_sync_in_worker_thread(  # type: ignore[return]
        ...
        limiter: abc.CapacityLimiteOptional[r] = None,
    ) -> T_Retval:
    ...
    def check_cancelled(cls) -> None:
        scope: CancelScopOptional[e] = threadlocals.current_cancel_scope
        while scope is not None:
    ...
    async def open_process(
        ...
        *,
        stdin: int | IO[AnOptional[y]],
        stdout: int | IO[AnOptional[y]],
        stderr: int | IO[AnOptional[y]],
        **kwargs: Any,
    ) -> Process:
        await cls.checkpoint()
    ...
    async def connect_tcp(
        cls, host: str, port: int, local_address: IPSockAddrTypOptional[e] = None
    ) -> abc.SocketStream:
    ...
    async def create_udp_socket(
        ...
        local_address: IPSockAddrTypOptional[e],
        remote_address: IPSockAddrTypOptional[e],
        reuse_port: bool,
    ...
    async def create_unix_datagram_socket(  # type: ignore[override]
        cls, raw_socket: socket.socket, remote_path: str | byteOptional[s]
    ...
    async def getaddrinfo(
        cls,
        host: bytes | stOptional[r],
        port: str | inOptional[t],
        *,
