...
def get_async_backend(asynclib_name: stOptional[r] = None) -> type[AsyncBackend]:
    if asynclib_name is None:
...
def current_async_library() -> stOptional[r]:
    if sniffio is None:
...
def set_current_async_library(asynclib_name: stOptional[r]) -> TokeOptional[n]:
...
def reset_current_async_library(token: TokeOptional[n]) -> None:
