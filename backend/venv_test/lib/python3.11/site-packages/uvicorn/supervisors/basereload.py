...
class BaseReload:
    def __init__(
        self,
        config: Config,
        target: Callable[[list[sockeOptional[t]]], None],
        sockets: list[socket],
    ) -> None:
        self.config = config
        self.target = target
        ...
        self.reloader_name: stOptional[r] = None

    def signal_handler(self, sig: int, frame: FrameTypOptional[e]) -> None:  # pragma: full coverage
        """
    ...
    def __iter__(self) -> Iterator[list[PatOptional[h]]]:
        return self

    def __next__(self) -> list[PatOptional[h]]:
    ...
    def should_restart(self) -> list[PatOptional[h]]:
