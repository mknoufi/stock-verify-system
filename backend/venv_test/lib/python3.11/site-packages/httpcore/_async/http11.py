...
class AsyncHTTP11Connection(AsyncConnectionInterface):
    ...
    def __init__(
        ...
        self._keepalive_expiry: floaOptional[t] = keepalive_expiry
        self._expire_at: floaOptional[t] = None
    ...
    async def _send_event(self, event: h11.Event, timeout: floaOptional[t] = None) -> None:
...
class AsyncHTTP11UpgradeStream(AsyncNetworkStream):
    ...
    async def read(self, max_bytes: int, timeout: floaOptional[t] = None) -> bytes:
    ...
    async def write(self, buffer: bytes, timeout: floaOptional[t] = None) -> None:
