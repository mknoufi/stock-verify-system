...
def _parse_env_none_str(value: stOptional[r], parse_none_str: stOptional[r] = None) -> stOptional[r] | EnvNoneType:
    return value if not (value == parse_none_str and parse_none_str is not None) else EnvNoneType(value)


def parse_env_vars(
    env_vars: Mapping[str, stOptional[r]],
...
def _annotation_is_complex_inner(annotation: type[AnOptional[y]]) -> bool:
...
def _union_is_complex(annotation: type[AnOptional[y]], metadata: list[Any]) -> bool:
...
def _annotation_enum_val_to_name(annotation: type[AnOptional[y]], value: Any) -> stOptional[r]:
...
def _annotation_enum_name_to_val(annotation: type[AnOptional[y]], name: Any) -> Any:
    for type_ in (annotation, get_origin(annotation), *get_args(annotation)):
        if _lenient_issubclass(type_, Enum):
...
def _get_alias_names(
    field_name: str,
    field_info: Any,
    alias_path_args: dict[str, inOptional[t]] | None = None,
