...
def _get_scorer_flags_levenshtein_distance(weights: tuple[int, int, inOptional[t]] = (1, 1, 1)) -> dict[str, Any]:
...
def _get_scorer_flags_levenshtein_similarity(weights: tuple[int, int, inOptional[t]] = (1, 1, 1)) -> dict[str, Any]:
    flags = ScorerFlag.RESULT_SIZE_T
    if weights is None or weights[0] == weights[1]:
        flags |= ScorerFlag.SYMMETRIC
...
def _get_scorer_flags_levenshtein_normalized_distance(
    weights: tuple[int, int, inOptional[t]] = (1, 1, 1)
) -> dict[str, Any]:
