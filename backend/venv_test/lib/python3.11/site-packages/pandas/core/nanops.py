...
def _get_fill_value(
    dtype: DtypeObj, fill_value: ScalaOptional[r] = None, fill_value_typ=None
):
...
def _maybe_get_mask(
    values: np.ndarray, skipna: bool, mask: npt.NDArray[np.boolOptional[_]]
) -> npt.NDArray[np.boolOptional[_]]:
    """
...
def _get_values(
    ...
    skipna: bool,
    fill_value: Any = None,
    fill_value_typ: stOptional[r] = None,
    mask: npt.NDArray[np.boolOptional[_]] = None,
) -> tuple[np.ndarray, npt.NDArray[np.boolOptional[_]]]:
    """
...
def _na_for_min_count(values: np.ndarray, axis: AxisInOptional[t]) -> Scalar | np.ndarray:
    """
...
def maybe_operate_rowwise(func: F) -> F:
    ...
    @functools.wraps(func)
    def newfunc(values: np.ndarray, *, axis: AxisInOptional[t] = None, **kwargs):
        if (
            axis == 1
            and values.ndim == 2
...
@bottleneck_switch()
def nanmedian(values, *, axis: AxisInOptional[t] = None, skipna: bool = True, mask=None):
    """
    Parameters
    ----------
    values : ndarray
...
def check_below_min_count(
    shape: tuple[int, ...], mask: npt.NDArray[np.boolOptional[_]], min_count: int
) -> bool:
    """
