...
def find_valid_index(how: str, is_valid: npt.NDArray[np.bool_]) -> inOptional[t]:
    """
...
def validate_limit_area(limit_area: stOptional[r]) -> Literal["inside", "outside"] | None:
...
def interpolate_2d_inplace(
    ...
    limit_area: stOptional[r] = None,
    fill_value: AnOptional[y] = None,
    mask=None,
    **kwargs,
) -> None:
    """
...
def _fillna_prep(
    values, mask: npt.NDArray[np.boolOptional[_]] = None
...
def clean_reindex_fill_method(method) -> ReindexMethoOptional[d]:
    if method is None:
        return None
...
def _interp_limit(
    invalid: npt.NDArray[np.bool_], fw_limit: inOptional[t], bw_limit: inOptional[t]
):
    """
