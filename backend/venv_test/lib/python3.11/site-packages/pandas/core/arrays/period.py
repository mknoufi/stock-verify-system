...
def _field_accessor(name: str, docstring: stOptional[r] = None):
    def f(self):
...
class PeriodArray(dtl.DatelikeOps, libperiod.PeriodMixin):  # type: ignore[misc]
    ...
    def __init__(
        self, values, dtype: DtypOptional[e] = None, freq=None, copy: bool = False
    ) -> None:
        if freq is not None:
            # GH#52462
            warnings.warn(
...
def period_array(
    data: Sequence[Period | stOptional[r]] | AnyArrayLike,
    freq: str | Tick | BaseOffseOptional[t] = None,
    copy: bool = False,
) -> PeriodArray:
    """
...
@overload
def validate_dtype_freq(dtype, freq: timedelta | stOptional[r]) -> BaseOffset:
    ...


def validate_dtype_freq(
    dtype, freq: BaseOffsetT | BaseOffset | timedelta | stOptional[r]
) -> BaseOffsetT:
    """
