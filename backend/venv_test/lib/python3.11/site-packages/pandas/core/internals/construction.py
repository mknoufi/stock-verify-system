...
def ndarray_to_mgr(
    values, index, columns, dtype: DtypeObOptional[j], copy: bool, typ: str
) -> Manager:
...
def _homogenize(
    data, index: Index, dtype: DtypeObOptional[j]
...
def reorder_arrays(
    arrays: list[ArrayLike], arr_columns: Index, columns: IndeOptional[x], length: int
...
def _get_axes(
    N: int, K: int, index: IndeOptional[x], columns: IndeOptional[x]
) -> tuple[Index, Index]:
...
def to_arrays(
    data, columns: IndeOptional[x], dtype: DtypeObOptional[j] = None
) -> tuple[list[ArrayLike], Index]:
    """
...
def _validate_or_indexify_columns(
    content: list[np.ndarray], columns: IndeOptional[x]
) -> Index:
    """
