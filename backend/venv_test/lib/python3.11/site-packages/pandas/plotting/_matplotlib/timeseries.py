...
def _upsample_others(ax: Axes, freq: BaseOffset, kwargs: dict[str, Any]) -> None:
    ...
    if legend is not None and kwargs.get("legend", True) and len(lines) > 0:
        title: stOptional[r] = legend.get_title().get_text()
        if title == "None":
            title = None
...
def _get_period_alias(freq: timedelta | BaseOffset | str) -> stOptional[r]:
...
def _get_index_freq(index: Index) -> BaseOffseOptional[t]:
...
def maybe_convert_index(ax: Axes, data: NDFrameT) -> NDFrameT:
    ...
    if isinstance(data.index, (ABCDatetimeIndex, ABCPeriodIndex)):
        freq: str | BaseOffseOptional[t] = data.index.freq
