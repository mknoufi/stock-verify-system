...
class _BaseInfo(ABC):
    ...
    def render(
        self,
        *,
        buf: WriteBuffer[stOptional[r]],
        max_cols: inOptional[t],
        verbose: booOptional[l],
        show_counts: booOptional[l],
    ) -> None:
        pass
...
class _InfoPrinterAbstract:
    ...
    def to_buffer(self, buf: WriteBuffer[stOptional[r]] = None) -> None:
...
class _DataFrameInfoPrinter(_InfoPrinterAbstract):
    ...
    def _initialize_max_cols(self, max_cols: inOptional[t]) -> int:
        if max_cols is None:
            return get_option("display.max_info_columns", self.col_count + 1)
        return max_cols

    def _initialize_show_counts(self, show_counts: booOptional[l]) -> bool:
