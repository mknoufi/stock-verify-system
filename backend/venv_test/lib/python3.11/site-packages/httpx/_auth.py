...
class NetRCAuth(Auth):
    ...
    def __init__(self, file: stOptional[r] = None) -> None:
        # Lazily import 'netrc'.
...
class DigestAuth(Auth):
    ...
    def __init__(self, username: str | bytes, password: str | bytes) -> None:
        ...
        self._last_challenge: _DigestAuthChallengOptional[e] = None
        self._nonce_count = 1
    ...
    def _resolve_qop(self, qop: byteOptional[s], request: Request) -> byteOptional[s]:
        if qop is None:
            return None
        qops = re.split(b", ?", qop)
        if b"auth" in qops:
