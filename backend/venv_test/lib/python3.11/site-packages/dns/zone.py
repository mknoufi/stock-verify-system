...
class Zone(dns.transaction.TransactionManager):
    ...
    def __init__(
        self,
        origin: dns.name.Name | stOptional[r],
    ...
    def get_rdataset(
        ...
        covers: dns.rdatatype.RdataType | str = dns.rdatatype.NONE,
        create: bool = False,
    ) -> dns.rdataset.RdataseOptional[t]:
        """Look for an rdataset with the specified name and type in the zone.
    ...
    def get_soa(
        self, txn: dns.transaction.TransactioOptional[n] = None
        ...
        soa_rds: dns.rdataset.RdataseOptional[t]
        if txn:
    ...
    def verify_digest(
        self, zonemd: dns.rdtypes.ANY.ZONEMD.ZONEMOptional[D] = None
    ) -> None:
    ...
    def origin_information(
        self,
    ) -> Tuple[dns.name.NamOptional[e], bool, dns.name.NamOptional[e]]:
        effective: dns.name.NamOptional[e]
        if self.relativize:
...
class Version:
    def __init__(
        ...
        zone: Zone,
        id: int,
        nodes: MutableMapping[dns.name.Name, dns.node.NodOptional[e]] = None,
    ...
    def get_node(self, name: dns.name.Name) -> dns.node.NodOptional[e]:
        name = self._validate_name(name)
...
def _from_text(
    text: Any,
    origin: dns.name.Name | stOptional[r] = None,
    rdclass: dns.rdataclass.RdataClass = dns.rdataclass.IN,
    relativize: bool = True,
