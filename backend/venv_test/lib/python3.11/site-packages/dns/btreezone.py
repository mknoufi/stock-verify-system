...
class Node(dns.node.Node):
    ...
    def __init__(self, flags: NodeFlagOptional[s] = None):
        super().__init__()
        if flags is None:
...
class Delegations(dns.btree.BTreeSet[dns.name.Name]):
    def get_delegation(self, name: dns.name.Name) -> Tuple[dns.name.NamOptional[e], bool]:
...
class Zone(dns.versioned.Zone):
    ...
    writable_version_factory: (
        Callable[[dns.zone.Zone, bool], dns.zone.VersioOptional[n]]
    ) = WritableVersion
    immutable_version_factory: Callable[[dns.zone.Version], dns.zone.VersioOptional[n]] = (
        ImmutableVersion
    )
