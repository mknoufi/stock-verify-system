...
SavedStateType = Tuple[
    ...
    dns.name.NamOptional[e],  # current_origin
    dns.name.NamOptional[e],  # last_name
    AnOptional[y],  # current_file
    int,  # last_ttl
    bool,  # last_ttl_known
    int,  # default_ttl
    bool,
...
class Reader:
    ...
    def __init__(
        ...
        force_name: dns.name.NamOptional[e] = None,
        force_ttl: inOptional[t] = None,
        force_rdclass: dns.rdataclass.RdataClasOptional[s] = None,
        force_rdtype: dns.rdatatype.RdataTypOptional[e] = None,
        default_ttl: inOptional[t] = None,
    ):
        self.tok = tok
        ...
        self.current_file: AnOptional[y] = None
            ...
            while 1:
                ...
                if token.is_eof():
                    ...
                    if c == "$TTL":
                        ...
                        token = self.tok.get()
                        new_origin: dns.name.NamOptional[e]
...
class RRSetsReaderManager(dns.transaction.TransactionManager):
    def __init__(
        self,
        origin: dns.name.NamOptional[e] = dns.name.root,
...
def read_rrsets(
    text: Any,
    name: dns.name.Name | stOptional[r] = None,
    ttl: inOptional[t] = None,
    rdclass: dns.rdataclass.RdataClass | stOptional[r] = dns.rdataclass.IN,
    default_rdclass: dns.rdataclass.RdataClass | str = dns.rdataclass.IN,
    rdtype: dns.rdatatype.RdataType | stOptional[r] = None,
    default_ttl: int | stOptional[r] = None,
    idna_codec: dns.name.IDNACodeOptional[c] = None,
    origin: dns.name.Name | stOptional[r] = dns.name.root,
    relativize: bool = False,
