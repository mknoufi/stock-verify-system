...
class _Node(Generic[KT, ET]):
    ...
    def get(self, key: KT) -> EOptional[T]:
    ...
    def insert_nonfull(self, element: ET, in_order: bool) -> EOptional[T]:
    ...
    def delete(
        self, key: KT, parent: Optional["_Node[KT, ET]"], exact: EOptional[T]
    ) -> EOptional[T]:
...
class Cursor(Generic[KT, ET]):
    ...
    def __init__(self, btree: "BTree[KT, ET]"):
        self.btree = btree
        self.current_node: _NodOptional[e] = None
        ...
        self.parked = False
        self.parking_key: KOptional[T] = None
...
class BTree(Generic[KT, ET]):
    ...
    def insert_element(self, elt: ET, in_order: bool = False) -> EOptional[T]:
    ...
    def get_element(self, key: KT) -> EOptional[T]:
        """Get the element matching *key* from the BTree, or return ``None`` if it
        does not exist.
        """
        return self.root.get(key)

    def _delete(self, key: KT, exact: EOptional[T]) -> EOptional[T]:
        self._check_mutable_and_park()
        cloned = self.root.maybe_cow(self.creator)
        if cloned:
            self.root = cloned
    ...
    def delete_key(self, key: KT) -> EOptional[T]:
    ...
    def delete_exact(self, element: ET) -> EOptional[T]:
