...
class _Pipeline(Generic[_InT, _OutT]):
    ...
    def len(self: _Pipeline[_InT, _NewOutLen], min_len: int, max_len: inOptional[t] = None) -> _Pipeline[_InT, _NewOutLen]:
    ...
    def datetime_with_tz(
        self: _Pipeline[_InT, datetime.datetime], tz: datetime.tzinfOptional[o]
    ) -> _Pipeline[_InT, datetime.datetime]:
...
def _check_func(
    func: Callable[[Any], bool], predicate_err: str | Callable[[], str], s: cs.CoreSchemOptional[a]
) -> cs.CoreSchema:
...
def _apply_step(step: _Step, s: cs.CoreSchemOptional[a], handler: GetCoreSchemaHandler, source_type: Any) -> cs.CoreSchema:
...
def _apply_transform(
    s: cs.CoreSchemOptional[a], func: Callable[[Any], Any], handler: GetCoreSchemaHandler
) -> cs.CoreSchema:
    if s is None:
...
def _apply_constraint(  # noqa: C901
    s: cs.CoreSchemOptional[a], constraint: _ConstraintAnnotation
) -> cs.CoreSchema:
