...
_GENERIC_TYPES_CACHE: ContextVar[GenericTypesCachOptional[e]] = ContextVar('_GENERIC_TYPES_CACHE', default=None)


class PydanticGenericMetadata(TypedDict):
    origin: type[BaseModeOptional[l]]  # analogous to typing._GenericAlias.__origin__
...
def _get_caller_frame_info(depth: int = 2) -> tuple[stOptional[r], bool]:
...
def get_args(v: Any) -> Any:
    pydantic_generic_metadata: PydanticGenericMetadatOptional[a] = getattr(v, '__pydantic_generic_metadata__', None)
...
def get_standard_typevars_map(cls: Any) -> dict[TypeVar, AnOptional[y]]:
...
def replace_types(type_: Any, type_map: Mapping[TypeVar, AnOptional[y]]) -> Any:
...
_generic_recursion_cache: ContextVar[set[stOptional[r]]] = ContextVar('_generic_recursion_cache', default=None)


@contextmanager
def generic_recursion_self_type(
    origin: type[BaseModel], args: tuple[Any, ...]
) -> Iterator[PydanticRecursiveReOptional[f]]:
...
def get_cached_generic_type_early(parent: type[BaseModel], typevar_values: Any) -> type[BaseModeOptional[l]]:
...
def set_cached_generic_type(
    ...
    origin: type[BaseModeOptional[l]] = None,
