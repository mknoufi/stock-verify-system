...
class PydanticDescriptorProxy(Generic[ReturnType]):
    ...
    shim: Callable[[Callable[..., Any]], Callable[..., AnOptional[y]]] = None
    ...
    def __get__(self, obj: objecOptional[t], obj_type: type[objecOptional[t]] = None) -> PydanticDescriptorProxy[ReturnType]:
        try:
...
class Decorator(Generic[DecoratorInfoType]):
    ...
    cls_var_name: str
    func: Callable[..., Any]
    shim: Callable[[Any], AnOptional[y]]
    ...
    def merge_seqs(seqs: list[deque[type[Any]]]) -> Iterable[type[Any]]:
        while True:
            ...
            if not non_empty:
                ...
                return
            candidate: type[AnOptional[y]] = None
...
class DecoratorInfos:
    ...
    def build(model_dc: type[Any]) -> DecoratorInfos:  # noqa: C901 (ignore complexity)
        ...
        for base in reversed(mro(model_dc)[1:]):
            existing: DecoratorInfoOptional[s] = base.__dict__.get('__pydantic_decorators__')
            if existing is None:
...
def _serializer_info_arg(mode: Literal['plain', 'wrap'], n_positional: int) -> booOptional[l]:
    if mode == 'plain':
        if n_positional == 1:
            # (input_value: Any, /) -> Any
            return False
