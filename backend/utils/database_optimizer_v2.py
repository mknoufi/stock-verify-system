...
class OptimizedDatabaseQuery(Generic[T]):
    ...
    async def find_one(self) -> Result[dict[str, Optional[Any]], Exception]:
    ...
    async def find_many(self) -> Result[list[dict[str, Any]], Exception]:
        ...
        return await safe_async_execute(
            _find_many(), f"find_many:{self.collection.name}"
        )
    ...
    async def aggregate(
        ...
        return await safe_async_execute(
            _aggregate(), f"aggregate:{self.collection.name}"
        )
...
class DatabaseQueryBuilder:
    ...
    async def bulk_upsert(
        ...
        return await safe_async_execute(
            _bulk_upsert(), f"bulk_upsert:{collection_name}"
        )

    async def indexed_lookup(
        self, collection_name: str, index_field: str, value: Any
    ) -> Result[dict[str, Optional[Any]], Exception]:
        """
