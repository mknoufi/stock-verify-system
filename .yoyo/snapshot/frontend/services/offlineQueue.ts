...
export function startOfflineQueue(client: AxiosInstance): void {
  ...
  if (!unsubscribeNetwork) {
    unsubscribeNetwork = useNetworkStore.subscribe((state, prev) => {
      ...
      if (online && !flushing) {
        flushOfflineQueue(client).then((res) => {
          if (__DEV__ && (res.processed > 0 || res.remaining >= 0)) {
            console.log(`OfflineQueue: flushed processed=${res.processed} remaining=${res.remaining}`);
          }
          if (res.processed > 0) {
  ...
  client.interceptors.response.use(
    ...
    async (error) => {
      ...
      if (!online || isNetworkError) {
        ...
        if (__DEV__) {
          console.warn('OfflineQueue: queued mutation', { id: item.id, method: item.method, url: item.url });
        }
        try {
