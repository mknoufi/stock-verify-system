...
export class EnhancedErrorHandler extends ErrorHandler {
  ...
  static async handleWithRecovery<T>(
    ...
    const { handleErrorWithRecovery } = require('./errorRecovery');
  ...
  static async handleNetworkError<T>(
    operation: () => Promise<T>,
    options: { maxRetries?: number; showAlert?: boolean } = {}
  ): Promise<T> {
    const { handleNetworkError } = require('./errorRecovery');
    return await handleNetworkError(operation, options);
  }
  ...
  static async handleDatabaseError<T>(
    operation: () => Promise<T>,
    options: { fallback?: () => Promise<T>; showAlert?: boolean } = {}
  ): Promise<T> {
    const { handleDatabaseError } = require('./errorRecovery');
    return await handleDatabaseError(operation, options);
  }
}
